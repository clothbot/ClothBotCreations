...
module watchstrap_adjustment_band(l=adjustment_band_l
    ...
    difference() {
        union() {
            ...
            color([0,1,0]) for(i=[1:2*pre_offset_hole_count]) {
                hull() {
                    translate( circle_point_at_length(bend_r,(i-1)*hole_spacing/2+band_offset+lead_in_l,rotate=-90,offset=[0,bend_r] ) )
                        cylinder(d=th,h=w,center=true,$fn=8);
                    translate( circle_point_at_length(bend_r,i*hole_spacing/2+band_offset+lead_in_l,rotate=-90,offset=[0,bend_r] ) )
                        cylinder(d=th,h=w,center=true,$fn=8);
                }
            }
            color([0,0,1]) for(i=[1:2*(hole_count-1)]) {
                hull() {
            ...
            color([1,0,1]) for(i=[1:2*(post_offset_hole_count-hole_count+1)]) {
                hull() {
                    translate( circle_point_at_length(bend_r,hole_offset+(i-1)*hole_spacing/2+band_offset+hole_spacing*(hole_count-1),rotate=-90,offset=[0,bend_r] ) )
                        cylinder(d=th,h=w,center=true,$fn=8);
                    translate( circle_point_at_length(bend_r,hole_offset+i*hole_spacing/2+band_offset+hole_spacing*(hole_count-1),rotate=-90,offset=[0,bend_r] ) )
                        cylinder(d=th,h=w,center=true,$fn=8);
                }
            }
            color([1,1,0]) hull() {
                translate( circle_point_at_length(bend_r,hole_offset+post_offset_hole_count*hole_spacing+band_offset,rotate=-90,offset=[0,bend_r] ) )
                    cylinder(d=th,h=w,center=true,$fn=8);
                translate( strap_end )
                    cylinder(d=th,h=w,center=true,$fn=8);
            }
